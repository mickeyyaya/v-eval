...
Design (2026-09-15, shipped in `core/render/templates/report.html.tmpl`): a masthead with the artifact kind, the revision in monospace, and a hero card holding the overall badge, the rule applied, and the blocking criteria as chips; a section navigation that stays at the top on wide screens and scrolls sideways on narrow ones, listing only the sections the report renders; a two-column facts grid that collapses to one column under 40rem; section headings that carry the count of rendered items; evidence entries as cards (tags for kind, isolation, and the supplied marker; the locator in monospace; the observation with its whitespace preserved; the provenance line last); forensic findings with a severity badge; zebra tables inside sideways-scrolling wrappers; a footer naming the report id, schema version, and v-eval version; and print rules that hide the navigation, unwrap the tables, and keep rows and cards on one page. Every color is a custom property on `:root` with a dark override that redefines exactly the same set, text on every badge and tag holds a contrast ratio of at least 4.5:1 in both themes, and the count chips present lengths of lists the report already carries, so the render still adds no information ([decision 0006](../decisions/0006-json-first-report-contract.md)).
